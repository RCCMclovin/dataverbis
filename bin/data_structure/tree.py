...
class Tree(object):
    ...
    def move_sub_tree(self, new_parent, node):
        ...
        if is_parent == False:
         ...
         new_parent.token_type == "FT"):
            ...
            new_parent.parent.children.remove(new_parent) #  = node.children.remove(new_parent) 
    ...
    def __repr__(self):
        result = ""
        result += "HashNum: " + str(self.hash_num) + " invalid: " + str(self.invalid) + " weight: " + str(round(self.weight,2)) + " cost: " + str(self.cost) + "\n"
        node_list = [self.root]
        level_list = [0]
        while node_list != []:
