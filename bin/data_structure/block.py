...
from misc.similarity import is_numeric
...
class Block(object):
    ...
    def translate(self, main_block, query_tree):
        ...
        for i in range(len(self.all_nodes)):
            ...
            if not cur_node.token_type == "NT" and len(cur_node.mapped_elements) > 0:
                ...
                condition += "." + cur_node.mapped_elements[cur_node.choice].schema_element.name 
                if cur_node.parent.token_type == "OT":
                    condition += " " + cur_node.parent.function + " " 

                elif cur_node.mapped_elements[cur_node.choice].choice == -1 and not is_numeric(cur_node.label):
                    condition += " LIKE \"%"
        ...
        if self == main_block:
            for i in range(len(self.inner_blocks)):
                inner_related = self.inner_blocks[i].find_related_node_from_self(main_block)
                #print(inner_related)
                ...
                if inner_related is not None and inner_related in self.inner_blocks[i].all_nodes:
                    ...
                    condition = left.to_string(main_block, "") + " = " +\
                     right.to_string(main_block, inner_related.mapped_elements[choice].schema_element.name) 
                    self.conditions.append(condition)
                
                elif inner_related is not None:
                    choice = inner_related.choice
                    left = SQLElement(main_block, inner_related) 
                    right = SQLElement(self.inner_blocks[i].inner_blocks[0], inner_related) 
                    condition = left.to_string(main_block, "") + " = " +\
                     right.to_string(main_block, inner_related.mapped_elements[choice].schema_element.name) 	
                    self.conditions.append(condition) 
    ...
    def __repr__(self):
        result = "block_" + str(self.block_id) + " root: " + str(self.block_root.label)
