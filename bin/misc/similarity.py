...
def similarity(tree_node, element,d):
    ...
    if is_numeric(node_label) and element.schema_element.type == 'number':
        #print node_label, max(tree_node.parent.mapped_elements), element.schema_element.name
        #print element.schema_element.relation.name, element.schema_element.name
        
        sum = 0
        for i in range(len(element.mapped_values)):
            sum += int(element.mapped_values[i])

        size = int(node_label) * len(element.mapped_values)
        element.similarity = 1.0 - float(abs(sum-size)/float(size))
        if tree_node.parent.label in d[element.schema_element.relation.name][element.schema_element.name]:
...
def if_list_similar(word,lst):
    sim = similarity_list(word,lst)
    if sim >= 0.6:
        return True
    return False

def similarity_list(word,lst):
    nsim = -1
    if word in lst:
        return 1
    for w in lst:
        sim = similarity_words(word, w)
        if sim > nsim:
            nsim = sim
